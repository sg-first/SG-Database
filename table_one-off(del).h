...
vector<int> read_from_len_file(const string& path){
    ...
    for(int i=0;i<len_data.length();i++){
        if(len_data[i]=='\n'){
             len_vec.push_back(stoi(len_data.substr(beg,i-beg)));
             beg=i+1;
        }
    }
    return len_vec;
}
void write_to_len_file(const string& path,const vector<int>& len_data){
    string len_path=path_to_lenpath(path);
    string len_string="";
    for(int i:len_data){
        len_string=len_string+to_string(i)+"\n";
    }
    fstream write(len_path,ios::out|ios::trunc);
    if (!write.is_open()) {
        cout<<"fail to open the file(write)"<<endl ;
    }
    write.seekp(0, ios::beg);
...
void do_add(vector<Basic*> turple,const string& path,vector<int>& len_data){
    string new_data="";
    ...
    len_data.push_back(new_data.length());
}
void do_del(int opSub,const string& path,vector<int>& len_data){
    int loc=0;
    for(int i=0;i<(opSub+1);i++){
        loc+=len_data[i];
    }
    fstream write(path);
    if (!write.is_open()) {
    ...
    write.seekp(loc+opSub+1,ios::beg);
    write << "#";
    write.close();
    len_data.erase(len_data.begin()+opSub+1);
}
class col
{
private:
...
public:
    ...
    string table_to_str(vector<int>& len_data){
    ...
    {
        vector<int> len_data=read_from_len_file(path);
